...
export class SecondaryWeaponRenderer {
  ...
  constructor(scene: THREE.Scene) {
    ...
    const garlicMaterial = this.trackMaterial(new THREE.MeshBasicMaterial({ color: 0x7fe0a2, transparent: true, opacity: 0.2, depthWrite: false }));
    const orbitMaterial = this.trackMaterial(new THREE.MeshStandardMaterial({ color: 0xd8f5ff, emissive: 0x548bb5, emissiveIntensity: 1.7 }));
    const ghostMaterial = this.trackMaterial(new THREE.MeshBasicMaterial({ color: 0xa7dfff, transparent: true, opacity: 0.65, depthWrite: false }));
  ...
  reset(): void { for (const mesh of [...this.garlic, ...this.orbits, ...this.ghosts]) mesh.visible = false; }
  dispose(): void {
  ...
  private syncPool(pool: THREE.Mesh[], states: readonly { active: boolean; position: { x: number; y: number; z: number }; radius: number }[]): void {
  ...
  private trackGeometry<T extends THREE.BufferGeometry>(value: T): T { this.geometries.push(value); return value; }
  private trackMaterial<T extends THREE.Material>(value: T): T { this.materials.push(value); return value; }
}
