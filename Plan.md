...
As of July 14, 2026, the complete nine-minute run is implemented through goals, halftime tactics, Blood Moon escalation, the Count Goalkeeper boss, and terminal results. Eight upgrades, two evolutions, four enemy archetypes plus elites, physical XP shards, procedural music, onboarding, diagnostics, and persistent settings are active. Browser automation, 48 deterministic tests, and real macOS Electron checks cover the main systems and desktop controls. Pointer-lock combat feel and full-run balance still require human playtesting. Windows packaging remains intentionally deferred until the game is content-complete.
