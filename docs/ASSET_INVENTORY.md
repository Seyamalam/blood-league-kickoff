...
- The repository contains seventeen authored SVG progression icons, seven authored JPEG screenshots, one generated key-art master, and two optimized key-art derivatives.
...
| Upgrade and evolution icons                                                   | `src/assets/progression/**/*.svg`     | Authored SVG/XML                       | Project-created original vector art       | Twelve upgrade and five evolution icons use the locked bone/semantic palette, transparent backgrounds, and distinct silhouettes; Vite imports and inlines them for offline runtime use |
...
All current screenshots are direct 1280×720 Chrome 149 captures: five from revision `dcf620b`, the alpha.2 results capture from `fb8e5bf`, and the gameplay capture from the Unreleased arena/enemy-readability passes. They were exported from PNG to JPEG with FFmpeg (`-q:v 2`). The title capture includes the generated key-art background recorded in the asset register; the remaining captures contain no generated or third-party raster imagery.
...
| `docs/screenshots/gameplay.jpg`        | DEV-only deterministic 72-enemy readability and performance scene | Project-created capture                                                  | SHA-256 `538721908e758a189004a8a88815696323bd39ebdfe8e000bf7f75f2c892a08a`                                   |
