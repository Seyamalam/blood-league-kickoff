...
Ordinary-enemy presentation uses three distance bands without changing simulation state. Near enemies use articulated block anatomy with archetype-specific tints, proportions, equipment, and attack poses. Mid/far enemies collapse to merged geometry that preserves the same voxel silhouette provenance, while throttled pose updates bound crowd cost. The former nearest-only imported-character pool is no longer instantiated. Bat Swarm retains its dedicated winged creature silhouette.

Character variants retain one lightweight articulated voxel skeleton. Runtime-owned kit, boot, hair, and role-equipment blocks mount to stable joints, so six distinct silhouettes and body proportions do not duplicate a full model. Shared box geometry and materials use reference-counted ownership. The same rig and code-driven animation controller power the world player and live Career preview; generated Codex portraits remain UI-only.
